...
$Button2 = GUICtrlCreateButton("Date retour", 0, 112, 265, 33)
...
;~    EndIf
				...
				Case $Button2 ;SAISIE DATE DE RETOUR SANS VF
					...
					if $ar[$i][1] = "A" or $ar[$i][1] = "B" Then ;CHOIX LIGNE DE SAISIE
						ControlSend($handle, "", "", "{F4}")
						Send("{DOWN}")
						sleep(200)
						GUISetState(@SW_HIDE, $Form1)

						$Form3 = GUICreate("Attente de la ligne de retour des résultat", 500, 100, 500, 180, $WS_POPUP, $WS_EX_TOPMOST)
						$Label31 = GUICtrlCreateLabel("Se positionner sur la ligne de retour et cliquer sur OK", 8, 8, 476, 30)
						GUICtrlSetFont(-1, 12, 800, 0, "MS Sans Serif")
						GUICtrlSetColor(-1, 0xFF0000)
						$Button31 = GUICtrlCreateButton("OK", 8, 40, 185, 33)
						$Button32 = GUICtrlCreateButton("Quitter", 258, 40, 185, 33)
						GUISetState(@SW_SHOW)
					...
					Else ;PAS DE CHOIX DE LIGNE DE SAISIE
						ControlSend($handle, "", "", "{F4}")
						sleep(200)
						Send("{DOWN}")
						Send("{DOWN}")
						...
						sleep(200)
					EndIf
					if $ar[$i][1] = "R" or $ar[$i][1] = "A" Then ; RETOUR UNIQUEMENT, PASSAGE DOSSIER SUIVANT
						Send("{F1}")
						if UBound($ar, 1) > 1 Then
							_ArrayDelete($ar, $i)
							_FileWriteFromArray(@ScriptDir & "\integration.txt", $ar, 0)
